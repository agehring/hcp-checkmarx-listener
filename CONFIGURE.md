...
2. **Create New Run Task**:
   ...
   | **Endpoint URL** | `http://your-service-host:80/run-task` |
...
```bash
curl http://your-service-host:80/health
```
...
1. **"Only pre_apply stage is supported" Error**:
   - Ensure your run task is configured for the **Pre-apply** stage
   - The service rejects post-plan and other stages
